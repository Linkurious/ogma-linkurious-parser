...
class EnhancedGitHubAgent extends GitHubAgent {
  ...
  _createBranch(name) {
    ...
    return this.get('git/refs/heads/master').then(({code, body}) => {
  ...
  pushFiles(parameters) {
    ...
    return this.get(`git/refs/heads/${branch}`).then(({code, body}) => {
      if (code === 404) {
  ...
  uploadRelease({tag_name, name, body, prerelease, zipName, zipPath}) {
    return this.post('releases', {tag_name, name, body, prerelease}).then(({body, code}) => {
      ...
      return new Promise(resolve => {
        fs.createReadStream(zipPath).pipe(request.post(options, function(err, res) {
          if (err) {
            throw err;
